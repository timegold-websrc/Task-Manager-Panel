...
(function() {
    ...
    GaugePointer = (function(superClass) {
        ...
        GaugePointer.prototype.render = function() {
            ...
            this.ctx.arc(0, 0, this.strokeWidth - this.strokeWidth / 5 * 2, 0, Math.PI * 2, false);
            this.ctx.fill();
            if (this.img) {
    ...
    Gauge = (function(superClass) {
            ...
            for (j = 0, len = ref.length; j < len; j++) {
                ...
                if (value.label !== void 0) {
                    ...
                    if ((!this.options.limitMin || value >= this.minValue) && (!this.options.limitMax || value <= this.maxValue)) {
                        ...
                        this.ctx.rotate(rotationAngle);
                        this.ctx.fillText(textValue[j], 0, -radius - this.lineWidth / 2);
                        this.ctx.rotate(-rotationAngle);
                    }
                }
            }
        ...
        Gauge.prototype.render = function() {
            var displayedAngle, fillStyle, gauge, h, j, l, len, len1, max, min, radius, ref, ref1, scaleMutate, tmpRadius, w, zone, subColor, subPos, angle;
            var subColors = ['#ff6549', '#ffba49', '#ffd849', '#35b796', '#4074b8'];
            var subLabels = ['VP', 'P', 'F', 'G', 'E'];
            ...
            if (this.options.staticZones) {
                ...
                for (j = 0, len = ref.length; j < len; j++) {
                    ...
                    this.ctx.beginPath();
                    this.ctx.strokeStyle = subColor;
                    this.ctx.arc(0, 0, tmpRadius - tmpRadius / 10 * 2, this.getAngle(min), this.getAngle(max), false);
                    this.ctx.stroke();

                    this.ctx.beginPath();
                    this.ctx.strokeStyle = 'white';
                    this.ctx.arc(0, 0, tmpRadius - tmpRadius / 10 * 8, this.getAngle(min), this.getAngle(max), false);
                    this.ctx.stroke();

                    subPos = (tmpRadius - tmpRadius / 10 * 8) - (tmpRadius - tmpRadius / 10 * 8) / 4;

                    this.ctx.beginPath();
                    this.ctx.strokeStyle = subColor;
                    this.ctx.arc(0, 0, (tmpRadius - tmpRadius / 10 * 8) - (tmpRadius - tmpRadius / 10 * 8) / 4, this.getAngle(min), this.getAngle(max), false);
                    this.ctx.stroke();

                    angle = this.getAngle(min + (max - min) / 2);
                    x = Math.round((tmpRadius - tmpRadius / 6 * 2) * Math.cos(angle) * 2);
                    if (j > 2) {
                        x = Math.round((tmpRadius - tmpRadius / 6 * 2) * Math.cos(angle) * 1.5);
                    }
                    if (j == 2) {
                        x = Math.round((tmpRadius - tmpRadius / 6 * 2) * Math.cos(angle) * 2);
                        x = x - 10;
                    }
                    y = Math.round(tmpRadius * Math.sin(angle));
                    this.ctx.beginPath();
                    this.ctx.font = "bold 20px Roboto";
                    this.ctx.fillStyle = 'white';
                    this.ctx.fillText(subLabels[j], x, y);

                }
                this.ctx.beginPath();
                this.ctx.fillStyle = "white";
                this.ctx.arc(0, 0, subPos * 4, 0, Math.PI * 2, false);
                this.ctx.fill();
